...
public interface BorrowrecordMapper extends MyMapper<Borrowrecord>{
    ...
    @Select("select * from  t_borrowrecord ")
    List<Borrowrecord> findAllborrow();

    @Insert("insert into t_borrowrecord(bid,biannum,name,place,supply,usernumber,borrowtime,backtime) values(#{bid},#{biannum},#{name},#{place},#{supply},#{usernumber},#{borrowtime},#{backtime})" )
    boolean addBorrowrecord(Borrowrecord borrowrecord);

    @Select("select * from  t_borrowrecord where usernumber=#{usernumber}")
    List<Borrowrecord> findAllMyborrow(String usernumber);

    @Select("select * from  t_borrowrecord where bid=#{bid}")
    Borrowrecord  findOneMyborrow(int bid);

    @Delete("delete from t_borrowrecord where bid=#{bid}")
